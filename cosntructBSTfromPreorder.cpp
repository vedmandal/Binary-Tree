...
node*construct_tree(vector<int>arr){
  ...
  int bst_idx = 0;
node *constructBSTFromPreOrder(vector<int> &arr, int lr, int rr)
{
    if (bst_idx == arr.size() || arr[bst_idx] < lr || arr[bst_idx] > rr)
        return nullptr;

    node *nod = new node(arr[bst_idx++]);
    nod->left = constructBSTFromPreOrder(arr, lr, nod->val);
    nod->right = constructBSTFromPreOrder(arr, nod->val, rr);
...
{
    return constructBSTFromPreOrder(preorder, -1e8, 1e8);
}
